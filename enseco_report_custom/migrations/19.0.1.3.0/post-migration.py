"""
Post-migrate de enseco_report_custom 19.0.1.3.0.
...
Vistas excluidas intencionalmente:
  - document_tax_totals (no funciona en v19, causa error al reactivar)
"""
import logging

_logger = logging.getLogger(__name__)

# Vistas que NO deben reactivarse (problemas conocidos en v19)
VISTAS_A_EXCLUIR = [
    'document_tax_totals',  # causa error de validación en v19
    'report_saleorder_document',  # id 4559 - xpaths v17 incompatibles
]


def migrate(cr, version):
    _logger.info(
        "enseco_report_custom post-migrate 19.0.1.2.0: "
    ...
    for view_id, xml_name in rows:
        excluida = " [EXCLUIDA]" if xml_name in VISTAS_A_EXCLUIR else ""
        _logger.info("    - enseco_report_custom.%s (id=%d)%s", xml_name, view_id, excluida)
    ...
    for view_id, xml_name in rows:
        ...
        # Saltar vistas excluidas intencionalmente
        if xml_name in VISTAS_A_EXCLUIR:
            _logger.info("    ⏭ %s (id=%d): excluida intencionalmente (no válida en v19)", xml_name, view_id)
            excluidas += 1
            continue
    ...
    _logger.info("    ⏭ Excluidas intencionalmente: %d", excluidas)
